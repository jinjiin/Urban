<template>
  <el-container class="full-height">
    <el-tabs type="border-card" tab-position="left" class="full-height-width">
      ...
      <el-tab-pane label="LSTM" class="full-height">
        <!-- SI-AGAN Steps-->
        <el-steps :active="GAN.active" simple>
          ...
          <el-step title="Finish" icon="el-icon-star-off"></el-step>
        </el-steps>
        <!-- SI-AGAN Start-->
        ...
        <div class="full-height-width" :style="getHeightStyle" v-show="GAN.seen==2">
          ...
          <div class="next-button-div">
            <el-button type="primary" @click="handle_train_model('LSTM')">Train</el-button>
          </div>
        </div>
        <!-- SI-AGAN Train-->
        ...
        <div class="full-height-width" :style="getHeightStyle" v-show="GAN.seen==4">
          <el-row class="center-div" style="width: 60%">
            ...
            -->
            <el-row>
              <span style="font-size:30px"><b>{{msg}}</b></span>
            </el-row>
            <br />
            <br />
            <br />
            <el-row>
              <el-button type="primary" @click="agan_reset('LSTM')" round>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Go Back &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</el-button>
            </el-row>
          </el-row>
        </div>
      </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    agan_reset(model_name) {
      this.GAN.active = 0;
      this.GAN.seen = 1;
      this.RemoveLog(model_name)
    },
    async handle_train_model(model_name) {
      var promise = this.SetModelParameter(model_name, this.GAN.params);
      await promise.then((response) => {
        console.log("Set Param success");
      })
      promise = this.TrainModel(model_name, this.global_.data_name, this.attribute);
      this.GAN.active = 2;
      this.GAN.seen = 3;
      var start_time_ms = new Date().getTime();
      var limit = 10 * 60 * 1000;

      function loopGetProgress(model_name, start, limit, self) {
        var pms = self.GetTrainProgress(model_name);
        pms.then((response) => {
      ...
      loopGetProgress(model_name, start_time_ms, limit, this);
    },
    si_agan_predict_one() {
