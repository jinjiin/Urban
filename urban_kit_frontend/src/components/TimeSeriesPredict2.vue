...
export default {
  ...
  data() {
    return {
      ...
      poi_options: [],
      attr_options: [{ value: "AQI", label: "AQI" }, { value: "no2", label: "NO2" }, { value: "o3", label: "O3" }, { value: "pm10", label: "PM10" }, { value: "pm25", label: "PM25" }, { value: "so2", label: "SO2" }],
    ...
    run() {
      ...
      promise.then((response) => {
        ...
        for (var i = 0; i < data[0].length; ++i) {
          ...
          raw.push(data[1][i]);
          predict.push(data[1][i]);
        }
        this.draw(timeline, raw, predict);
      });
    },
    draw(timeline, raw, predict) {
      var option = {
        tooltip: {
          trigger: 'axis',
          axisPointer: {
