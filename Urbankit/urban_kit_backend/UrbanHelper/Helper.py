...
def GetTrainProgress(model_name):
    ...
    if not latest_log or len(latest_log) == 0 or len(latest_log[-1].split(",")) != 2:
        return last_p
    latest_log = latest_log[-1].strip().split(',')
    s1, s2 = latest_log[0], latest_log[1]
    if model_name == 'LSTM':
        p = int(round(float(s1) + float(s2)))
    else:
        p = 0
        if s1 == "MeanShift":
            p = min(10, last_p + 1)
...
def PredictTimeSeries(data_name, attr_name, id):
    ...
    if not attr_name:
        datas = []
        for attr_name in ['AQI', 'no2', 'o3', 'pm10', 'pm25', 'so2']:
            datas.append(SinglePredictTimeSeries(data_name, attr_name))
        return [list(range(len(datas[0])))] + datas
    else:
        data = SinglePredictTimeSeries(data_name, attr_name)
...
def SinglePredictTimeSeries(method, data_name):
    data = np.load('UrbanModels/Temp/data_citys_1920/npy_data/%s.npy' % data_name)
    return data.flatten().tolist()[:2000]
